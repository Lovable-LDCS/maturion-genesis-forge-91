...
doc_counts AS (
  SELECT dom.organization_id,
         COUNT(DISTINCT dom.document_id)                                  AS total_documents,
         COUNT(c.id)                                                     AS total_chunks_attached
  FROM doc_org dom
  LEFT JOIN public.ai_document_chunks c ON c.document_id = dom.document_id
  GROUP BY dom.organization_id
)
