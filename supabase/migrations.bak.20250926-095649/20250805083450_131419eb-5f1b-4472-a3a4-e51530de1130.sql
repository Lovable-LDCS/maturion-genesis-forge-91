...
-- Create updated_at trigger for qa_rules
CREATE TRIGGER update_qa_rules_updated_at
  BEFORE UPDATE ON public.qa_rules
  FOR EACH ROW
...
-- Insert default QA rules for organizations
...
SELECT 
  ...
  '{"threshold": 95, "metric": "upload_success_rate", "timeframe": "1h"}',
  'warning',
  o.owner_id,
  o.owner_id
...
SELECT 
  ...
  '{"threshold": 10, "metric": "avg_processing_time", "timeframe": "1h"}',
  'info',
  o.owner_id,
  o.owner_id
FROM public.organizations o
...
SELECT 
  ...
  '{"threshold": 5, "metric": "rls_errors", "timeframe": "5m"}',
  'critical',
  o.owner_id,
  o.owner_id
FROM public.organizations o
