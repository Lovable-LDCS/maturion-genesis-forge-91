...
drop trigger if exists log_org_domains_audit_trail on public.org_domains;
...
drop trigger if exists log_org_crawl_queue_audit_trail on public.org_crawl_queue;
create trigger log_org_crawl_queue_audit_trail
  after insert or update or delete on public.org_crawl_queue
  for each row execute function public.log_audit_trail();

drop trigger if exists log_org_pages_audit_trail on public.org_pages;
create trigger log_org_pages_audit_trail
  after insert or update or delete on public.org_pages
  for each row execute function public.log_audit_trail();

drop trigger if exists log_org_ingest_jobs_audit_trail on public.org_ingest_jobs;
