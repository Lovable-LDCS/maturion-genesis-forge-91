...
CREATE INDEX IF NOT EXISTS ai_docs_stage_idx ON ai_documents(stage) WHERE stage IS NOT NULL;
CREATE INDEX IF NOT EXISTS ai_docs_layer_idx ON ai_documents(layer) WHERE layer IS NOT NULL;
CREATE INDEX IF NOT EXISTS ai_docs_doc_type_idx ON ai_documents(doc_type) WHERE doc_type IS NOT NULL;
CREATE INDEX IF NOT EXISTS ai_docs_processing_status_idx ON ai_documents(processing_status);

-- GIN indexes for arrays and JSONB - correct syntax for PostgreSQL
CREATE INDEX IF NOT EXISTS ai_docs_metadata_gin ON ai_documents USING gin(metadata);
...
CREATE INDEX IF NOT EXISTS ai_chunks_layer_idx ON ai_document_chunks(layer) WHERE layer IS NOT NULL;
CREATE INDEX IF NOT EXISTS ai_chunks_stage_idx ON ai_document_chunks(stage) WHERE stage IS NOT NULL;
