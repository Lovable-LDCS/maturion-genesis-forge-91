    ...
    try {
      ...
      if (action === 'approve') {
        ...
        if (deferral) {
          ...
          const { data: targetDomainData } = await supabase
            .from('domains')
            .select('id')
            .eq('organization_id', organizationId)
          ...
          if (targetDomainData) {
            ...
            const { data: suggestedMPS } = await supabase
            ...
            if (suggestedMPS) {
              ...
              console.log(`✅ Moved criterion from ${deferral.sourceDomain} to ${deferral.targetDomain} MPS ${deferral.targetMPS}`);
            } else {
              console.error('Could not find suggested MPS');
              return { success: false };
            }
          } else {
            console.error('Could not find target domain');
            return { success: false };
          }
        }
      }
